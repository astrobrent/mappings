...
c
      double precision T_av(mxdust,mxdtype),norm1,norm2
      double precision Tnorm(mxdtype), Tsignorm(mxdtype)
c
...
c
 ...
 1002 format('Average Dust Temp: ',1pg14.7)
      ...
      if (IRTemp) then
         do dtype=1,numtypes
           ...
           enddo
           do l=mindust(dtype),maxdust(dtype)
             Tnorm(dtype)=Tnorm(dtype)+
     &             T_av(l,dtype)*dustnum(l,dtype)*d_rad(l) 
             Tsignorm(dtype)=Tsignorm(dtype)+
     &             T_av(l,dtype)*dustsig(l,dtype)*d_rad(l) 
           enddo
 ...
 105       format(i4,2x,2(1pg15.7,2x),2x,2(1pg15.7,2x))
           write(15,105) IRcount, Tnorm(1),Tsignorm(1), 
     &                   Tnorm(2), Tsignorm(2)
           close(15)
      endif
