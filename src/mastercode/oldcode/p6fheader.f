...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c     
      subroutine p6fheader(newfil,banfil,fnam,
...
c     
 ...
 410  format(' Photoionisation model P5'/
 ...
 2222    format('ABUND:',16(a2,':',1pg10.3,x)//)
      endif
c
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
c
...
c
      if (grainmode) then
        if(gdist.eq.'M') then
          write(luop, 630) galpha, amin(1), amax(1),amin(2),  amax(2) 
        else if(gdist.eq.'P') then
          write(luop, 632) galpha, amin(1), amax(1), amin(2), amax(2),
     &          graindens(1), graindens(2), Bgrain, Yinf 
        endif
      endif
 630  format(//,' Dust Parameters :'/,
     &         ' -----------------'/,
     &         ' MRN size distribution'/,
     &         ' alpha   ',' Graphite min:    max: ',
     &         '   Silicate min:    max: '/,1pg10.3,2(4x,1pg10.3,1pg10.3))
c
 632  format(//,' Dust Parameters :'/,
     &         ' -----------------'/,
...
c'QHDNin',5x,
 440  format(/,3x,'#',3x,'Teav',8x,'DLOSav',6x,'Rav',7x,'Rout',8x,'dR',
     &   6x,'H.den',6x,'el.den',6x,'X_HI',5x,'X_HII',6x,'QHDNin',5x,
     &   'QHDNav',3x,'Caseab H',2x,'Caseab HeII')
...
c
 200  format(//'Dist.',t14,'Te',t26,'de',t38,'dh',t50,'en',t62,
     ...
     &     'Photo.',t254,'Recomb.',t266,'Cosmic',t278,'Grains',t290,
     &     'PAHs',//)
      write(lups,200)
c
      close(lups) 
