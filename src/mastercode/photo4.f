...
c
      ...
      double precision rstsun,vstrlo,wid,astar
      double precision presreg,tinner,dh10, starlum
c
      integer*4 j,luin,i
c
      character carac*36,where*16
      character caract*4,ilgg*4,fnam*80
...
c     
         ...
         if (ilgg.eq.'p') ilgg = 'P'
c     
         if ((ilgg.ne.'R').and.(ilgg.ne.'L').and.(ilgg.ne.'P')) goto 810
c     
         If (ilgg.eq.'R') then
c     
...
c     
         if (ilgg.eq.'D') ilgg = 'd'
         if (ilgg.eq.'Q') ilgg = 'q'
         if ((ilgg.ne.'q').and.(ilgg.ne.'d')) goto 312
c     
         if (ilgg.eq.'q') then  
 314        write(*,321)
...
c     
 ...
 849       write(*,850)
 850       format(/,' Set the convergence criteria, the maximum changes'
     &         ,'allowed',/
